 ...
 *
 * **HP 는 높이고 화력은 높이지 않는다.** 보스는 액터가 하나라 파티 인원이 적을수록
 * 한 사람이 받는 몫이 커진다 — 화력을 올리면 길어지는 게 아니라 그냥 진다.
 * 오래 버티되 덜 아픈 쪽이 "긴 전투" 를 만든다.
 *
 * ⚠️ **예전 값(HP 8, 화력 1.4)은 파티가 4인이던 시절의 것이다** (T-049a).
 * 실제 인원 2명으로 재보니 모든 레벨에서 승률이 0% 였다. 실측:
 *
 * ```
 * HP  화력    Lv5      Lv20     Lv40    승률폭
 *  3   1.0   100%10R  100%13R   82%14R    19%   ← 여기
 *  3   1.2    99%11R   56%14R   17%12R    82%
 *  4   1.0   100%14R   63%18R   12%16R    88%
 *  5   1.0    63%19R    1%20R    1%17R    62%
 * ```
 */
