 ...
 * 레벨업이 회복을 겸하므로(`LEVEL_UP_RECOVERY`) 초반 레벨업 간격이 곧 유적에서
 * 버티는 길이가 된다.
 *
 * ⚠️ **지수 1.8 은 회복 간격도 함께 벌린다.** 레벨이 높을수록 레벨업이 드물어지므로,
 * 레벨업 회복이 후할수록 "성장할수록 소모전에 약해지는" 구간이 생긴다 — T-049b 에서
 * 실제로 그랬다 (파티 Lv6 → Lv8 전멸률 25% → 35%). 회복을 일부로 줄여 없앴지만,
 * **이 곡선을 만질 때는 회복량과 함께 봐야 한다.**
 *
