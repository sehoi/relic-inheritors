 ...
 *
 * **슬롯을 정확히 채운다** (T-062). 파티가 넷이 되며 슬롯이 4 → 8 로 늘었는데 시작 유물이
 * 여섯 그대로라 두 칸이 비어 있었다 — 빈 슬롯은 고민이 아니라 그냥 손해다.
 *
 * 슬롯보다 **많이** 주지는 않는다. 유물이 12종뿐이라 시작에 아홉을 주면 유적에 남길 것이
 * 셋뿐이고, 그러면 회수 지점 넷(GDD §6.1)을 채울 수 없다. 대신 **첫 회수 지점을 줍는
 * 순간 아홉이 되어 "무엇을 뺄까" 가 시작된다** — 조합의 고민을 유적이 주는 편이
 * 처음부터 쥐여주는 것보다 이 게임의 말과 맞다.
 *
