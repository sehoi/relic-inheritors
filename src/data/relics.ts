 ...
 *
 * **슬롯 수보다 많아야 한다.** 지닌 유물이 슬롯 이하면 전부 끼는 것이 유일한 답이고,
 * 그러면 장착 화면(T-029)에 고를 것이 없다.
 *
 * ⚠️ **지금 그 조건이 깨져 있다.** 파티가 넷이 되며 슬롯이 4 → 8 로 늘었는데(T-049b)
 * 시작 유물은 여섯 그대로다 — 다 껴도 두 칸이 남는다. 여기 수를 늘리면 초반 화력이
 * 함께 오르므로 밸런스를 다시 재야 한다. 백로그에 남겨뒀다.
 *
