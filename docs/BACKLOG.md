...
- [ ] **T-016b** 아이템 커맨드
  - DoD: 파티 인벤토리를 `BattleState` 에 두고 수량 소비. 회복·상태해제·정화석. 수량 0이면 커맨드 거부
  - 인벤토리 자체는 M4(거점)와도 얽히므로, 전투 안에서 쓰는 최소 형태로만 만든다
...
- [ ] **밸런스 결정 필요: 침식 임계의 레벨 스케일링** ⬅ 사람이 처리
  - 보스전 승률이 Lv5 99% → Lv20 42% → Lv40 37% 로 급락한다. 원인은 침식 임계가 고정값(100)이라 전투가 길어지는 후반에 폭주가 누적되는 것 (폭주 147 → 676 → 724회)
  - 선택지: (a) 임계를 레벨/최대MP에 비례시킨다 (b) 스킬 침식량을 낮춘다 (c) 후반이 어려운 것을 의도로 둔다
  - **이 결정 전에는 M3 유물 밸런스를 잡을 수 없다** — 유물 수치가 전부 이 위에 쌓인다
- [ ] **T-020** 전투 씬 UI
  - DoD: 프론트뷰 배치, HP·침식 바, 커맨드 창, 대상 선택 커서, `BattleEvent` 로그를 애니메이션으로 재생. 스모크가 전투 1회를 끝까지 진행
- [ ] **T-021** 인카운터 연결
  - DoD: 걸음 수 카운터 + 지역별 인카운터 테이블, 탐색 ↔ 전투 씬 전환과 복귀. 전투 후 위치·상태 유지
