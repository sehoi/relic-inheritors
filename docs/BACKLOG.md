...
- [ ] **T-014** 스탯 계산과 데미지 해석
  - DoD: ADR-009 공식 구현(`PIERCE`·`DEF_FACTOR`·`variance`·`critMod` 전부 `data`). 관통 하한이 실제로 무적을 막는지 검증하는 테스트
- [ ] **T-015** 기본 커맨드 — 공격 / 방어 / 도망
  - DoD: 대상 선택 규칙, 방어의 피해 감소, 도망 성공률(AGI 기반, 시드 RNG). 승리·패배 판정
- [ ] **T-016** 스킬·아이템과 자원
  - DoD: 침식 누적과 임계 도달 시 폭주·봉인, MP 소비. 자원 부족 시 커맨드 거부
