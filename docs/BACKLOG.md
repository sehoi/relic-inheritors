...
- [ ] **T-017** 상태이상 5종 (독·마비·수면·침묵·혼란)
  - DoD: 턴 카운트 감소, 해제 조건, 중복 부여 규칙. 각 이상이 커맨드 선택을 어떻게 제약하는지 테스트
- [ ] **T-018** 적 AI
  - DoD: 가중치 기반 행동 선택 + HP 임계값 페이즈 전환. 시드 고정 시 재현
- [ ] **T-019** 밸런스 시뮬레이터 + 불변식 테스트 ⬅ **M3의 선행 조건**
  - DoD: `tools/sim.ts` 로 헤드리스 전투 N회 실행, GDD §5.5 불변식 7개를 검사하는 `tests/balance/`. `npm run sim` 동작
  - **이것 없이 유물·공명 데이터를 늘리지 않는다** (CLAUDE.md 규칙 9)
  - ⛔ **이 태스크를 마치면 루프를 멈추고 사용자에게 보고한다.** 불변식 임계값의 실제 측정치가 처음 나오는 지점이고, 그 숫자가 M3에서 루프가 수렴할 방향을 정한다. 사람이 한 번 봐야 한다
- [ ] **T-020** 전투 씬 UI
