...
`assets/index.json` 의 모든 항목(`packs`, `entries`)은 여기에 대응하는 줄이 있어야 한다 —
없으면 테스트가 실패한다 (ADR-006, ADR-007).

## 벤더 팩

통째로 들여온 제3자 에셋 디렉터리. 각 팩의 원본 라이선스 파일도 함께 보관한다.

| 팩 | 출처 | 라이선스 | 원본 라이선스 파일 |
|---|---|---|---|
...
`packs` 밖에서 낱개로 들여온 것. 현재는 없다 — 모두 벤더 팩 소속이다.
